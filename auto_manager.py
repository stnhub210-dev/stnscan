...
def dlb(fid): return f'<a href="https://drive.usercontent.google.com/download?id={fid}&export=download" class="dlb" title="다운로드">⬇</a>' if fid else ""
...
def generate_html(lawsuit_dict,ids):
    ...
    lr="".join(f"""<tr{ra(d['date'])}><td class="c">{i+1}</td><td class="c dc">{d['date']}{nb(d['date'])}</td><td>{d['info'][0]}</td><td>{d['info'][1]}</td><td class="c">{d['info'][2]}</td><td class="c">{d['info'][3]}</td><td class="c">{ub(d['urgency'])}</td><td><a href="{d['link']}" target="_blank" class="fl">📄 {d['fname']}</a>{dlb(d['fid'])}</td></tr>\n""" for i,d in enumerate(ld))
    ar="".join(f"""<tr{ra(d['date'])}><td class="c">{i+1}</td><td class="c dc">{d['date']}{nb(d['date'])}</td><td class="c">{'⚖️ 소송' if d['is_lawsuit'] else '📋 업무'}</td><td class="c">{d['company'] if not d['is_lawsuit'] else d['info'][0]}</td><td class="c">{d['info'][3] if d['is_lawsuit'] else d['doctype']}</td><td class="c">{ub(d['urgency']) if d['is_lawsuit'] else ''}</td><td><a href="{d['link']}" target="_blank" class="fl">📄 {d['fname']}</a>{dlb(d['fid'])}</td></tr>\n""" for i,d in enumerate(all_docs))
    wr="".join(f"""<tr{ra(d['date'])}><td class="c">{i+1}</td><td class="c dc">{d['date']}{nb(d['date'])}</td><td class="c">{d['company']}</td><td class="c">{d['doctype']}</td><td><a href="{d['link']}" target="_blank" class="fl">📄 {d['fname']}</a>{dlb(d['fid'])}</td></tr>\n""" for i,d in enumerate(wd))
    ...
    for cname,_ in COMPANIES:
        ...
        rows="".join(f"""<tr{ra(d['date'])}><td class="c">{i+1}</td><td class="c dc">{d['date']}{nb(d['date'])}</td><td class="c">{d['doctype']}{' '+ub('주의') if d['is_lawsuit'] else ''}</td><td><a href="{d['link']}" target="_blank" class="fl">📄 {d['fname']}</a>{dlb(d['fid'])}</td></tr>\n""" for i,d in enumerate(docs))
